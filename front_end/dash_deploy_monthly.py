...
def query_sector_by_area(supersector, industry,
                            beg_year, beg_month, 
                                end_year, end_month,
                                nplot): #industry, beg_year, beg_month, end_year, end_mont
    ...
    table_1 = sqlio.read_sql(f'''SELECT area,sum FROM

                   (SELECT area,date,SUM(value) AS sum FROM monthly
                   WHERE supersector='{supersector}'
                   AND industry='{industry}'
                   AND datatype='all employees, in thousands'
                   GROUP BY area,date) AS query_1

                   WHERE date='{real_beg_date}'
                   AND area != 'statewide'
                   ORDER BY area
                   ''', con)


    table_2 = sqlio.read_sql(f'''SELECT area,sum FROM 

                  (SELECT area,date,SUM(value) AS sum FROM monthly
                   WHERE supersector='{supersector}'
                   AND industry='{industry}'
                   AND datatype='all employees, in thousands'
                  GROUP BY area,date) AS query_2

                  WHERE date='{real_end_date}'
                  AND area != 'statewide'
                  ORDER BY area
                  ''', con)

    table_3 = table_2.merge(table_1, on='area')
    table_3["difference"] = table_3["sum_x"] - table_3["sum_y"]
    ...
    for locale in zip(range(1,len(tops)+1), tops):
        loc_frame_to_plot = sqlio.read_sql(f'''SELECT date,sum FROM 

                                (SELECT date,SUM(value) AS sum FROM monthly
                                WHERE supersector='{supersector}'
                                AND industry='{industry}'
                                AND datatype='all employees, in thousands'
                                AND area='{locale[1]}'
                                GROUP BY date) AS query_3

                                WHERE date BETWEEN '{real_beg_date}' AND '{real_end_date}'
                                ORDER BY date
                                ''', con)

        dates = loc_frame_to_plot["date"]
        sums = loc_frame_to_plot["sum"]
...
def update_figure(supersector, industry, beg_year, beg_month, end_year, end_month, howmany):
    return(query_sector_by_area(supersector, industry, beg_year, beg_month, end_year, end_month, howmany))
